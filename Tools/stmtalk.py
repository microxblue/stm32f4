...
class STM32_COMM:
    ...
    TARGET_CFGS = {
      ...
      1: {'blocksize':0x00004000, 'pagesize':0x00000040, 'base':0x08000000, 'limit':0x08100000, 'progcmd':"fp  "},  # IROM
    ...
    def speed_test (self, plen, test_time):
        ...
        if (ret != plen) :
            ...
            print ("USB write test speed %8.2f KB/S!" % (plen * loop / 1024.0 / (tick2 - tick1)))

        time.sleep(0.1)
        if (self.shell_cmd  (test_cmds['r'])) :
            raise SystemExit ("ERR: failed to send shell command")
        ...
        if (len(data) != plen) :
            ...
            print ("USB read  test speed %8.2f KB/S!" % (plen * loop / 1024.0 / (tick2 - tick1)))
...
def main():
    ...
    if result[7] == 1:
        # STM32F407
        stm_comm.set_mcu ('stm32f407')
    else:
        # STM32F103
        stm_comm.set_mcu ('stm32f103')
